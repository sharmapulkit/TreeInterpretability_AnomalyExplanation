...
sbatch --partition=defq --job-name=job1 ~/nrun_inf.sh python postGresRandomBaselineRankCorrelation.py --model_dir=$modelPath --test_dir=$dataPath'interpretations_shuffleFixed/' --train_dir=$dataPath'Subset/Train_subset' --outdir=$dataPath'diffInterpretations_templates_randomBaseline_shuffleFixed/'
